...
module Vcard
  ...
  # if used with care.
  #
  # All line values can be accessed with Base#value, Base#values, or even by
  # iterating through Base#lines. Line types that don't have specific support
  ...
  class Base < DirectoryInfo
    ...
    class Name
      ...
      def encode #:nodoc:
         Vcard::DirectoryInfo::Field.create('N',
           Vcard.encode_text_list([ @family, @given, @additional, @prefix, @suffix ].map{|n| n.strip}, ';')
           )
      end
      def encode_fn #:nodoc:
        fn = @fullname.strip
    ...
    def decode_list_of_text(field) #:nodoc:
      Line.new( field.group, field.name,
               Vcard.decode_text_list(field.value_raw).select{|t| t.length > 0}.uniq
              )
    end
    ...
    def decode_agent(field) #:nodoc:
      ...
      when 'uri'
        decode_uri(field)
      when 'vcard', nil
        Line.new( field.group, field.name, Base.decode(Vcard.decode_text(field.value_raw)).first )
      else
    ...
    def Base.create(fields = [] )
      fields.unshift Field.create('VERSION', "3.0")
      super(fields, 'VCARD')
    end

    # Decode a collection of vCards into an array of Base objects.
    #
    ...
    #
    #   vcard = Base.decode(card_data).first
    #
    # Note: Should the import encoding be remembered, so that it can be reencoded in
    # the same format?
    def Base.decode(card)
      if card.respond_to? :to_str
        string = card.to_str
      elsif card.respond_to? :read
        string = card.read(nil)
      else
        raise ArgumentError, "Base.decode cannot be called with a #{card.type}"
      end
    ...
    # The AGENT values. Each AGENT value is either a String, a Uri, or a Base.
    ...
    #
    # Yields a Vcard::Base::Maker that can be used to modify this vCard.
    def make #:yield: maker
      Vcard::Base::Maker.make2(self) do |maker|
        yield maker
      end
    end
    ...
    #
    # It can be used to create completely new vCards using Base#make2.
    #
    # Its is also yielded from Vcard::Base#make, in which case it allows a kind
    ...
    # - link:ex_cpvcard.txt: example of copying and them modifying a vCard
    # - link:ex_mkv21vcard.txt: example of creating version 2.1 vCard
    # - link:ex_mkyourown.txt: example of adding support for new fields to Base::Maker
    class Maker
      # Make a vCard.
      #
      # Yields +maker+, a Vcard::Base::Maker which allows fields to be added to
      # +card+, and returns +card+, a Vcard::Base.
      #
      # If +card+ is nil or not provided a new Vcard::Base is created and the
      ...
      #   information in N, see Base::Name#preformatted for more information.
      # - vCards must have a VERSION field. If one does not exist when your block is
      #   is finished it will be set to 3.0.
      def self.make2(card = Vcard::Base.create, &block) # :yields: maker
      ...
      def self.make(full_name = nil, &block) # :yields: maker
        new(full_name, Vcard::Base.create).make(&block)
      end
      ...
      def make # :nodoc:
        ...
        unless fn
          @card << Vcard::DirectoryInfo::Field.create('FN', Vcard::Base::Name.new(@card['N'], '').formatted)
        end
        unless @card['VERSION']
      ...
      # to Base::Name#formatted if Base::Name#fullname isn't given a specific
      # value.
      #
      ...
      # Add an address field, ADR. +address+ is a Vcard::Base::Address.
      def add_addr # :yield: address
        x = Vcard::Base::Address.new
        yield x
        @card << x.encode
        self
      end

      # Add a telephone field, TEL. +tel+ is a Vcard::Base::Telephone.
      #
      # The block is optional, its only necessary if you want to specify
      # the optional attributes.
      def add_tel(number) # :yield: tel
        x = Vcard::Base::Telephone.new(number)
        if block_given?
          yield x
        end
        @card << x.encode
      ...
      # Add an email field, EMAIL. +email+ is a Vcard::Base::Email.
      #
      # The block is optional, its only necessary if you want to specify
      # the optional attributes.
      def add_email(email) # :yield: email
        x = Vcard::Base::Email.new(email)
        if block_given?
          yield x
        end
        @card << x.encode
      ...
      def add_impp(url) # :yield: impp
        ...
        service_types = { skype: 'X-SKYPE', yahoo: 'X-YAHOO', icq: 'X-ICQ', google_talk: 'X-GTALK', msn: 'X-MSN' }

        field = 'IMPP'

        if block_given?
          x = Struct.new( :location, :preferred, :purpose, :service_type ).new
          ...
          field = service_types[x[:service_type].to_s.to_sym] if service_types[x[:service_type].to_s.to_sym].present?
        end

        @card << Vcard::DirectoryInfo::Field.create( field, url, params)
        self
      end
      ...
      def company=(company)
        @card << Vcard::DirectoryInfo::Field.create( 'X-ABShowAs', 'COMPANY' ) if company
      end
      ...
      def add_url(url)
        ...
        @card << Vcard::DirectoryInfo::Field.create( 'URL', url.to_s, params)
        self
      end
      ...
      def delete_if #:yield: line
        ...
        @card.delete_if do |line|
          ...
          # be passed instead of a Base, and for it to almost work. Yuck.
        end
      end
