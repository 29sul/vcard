...
module Vcard
  ...
  def Vcard.decode_list(value, sep = ',') # :nodoc:
    list = []
  ...
  def Vcard.encode_date(d) # :nodoc:
     "%0.4d%0.2d%0.2d" % [ d.year, d.mon, d.day ]
  end

  # Encode a Date object as "yyyymmdd".
  def Vcard.encode_time(d) # :nodoc:
     "%0.4d%0.2d%0.2d" % [ d.year, d.mon, d.day ]
  end

  # Encode a Time or DateTime object as "yyyymmddThhmmss"
  def Vcard.encode_date_time(d) # :nodoc:
  ...
  def Vcard.decode_time_to_time(v) # :nodoc:
  ...
  def Vcard.decode_date_time(v) # :nodoc:
  ...
  def Vcard.decode_date_time_to_datetime(v) #:nodoc:
    year, month, day, hour, min, sec, secfrac, tz = Vcard.decode_date_time(v)
    # TODO - DateTime understands timezones, so we could decode tz and use it.
    DateTime.civil(year, month, day, hour, min, sec, 0)
  end

  # Vcard.decode_boolean
  #
  # float
  #
  # float_list
...
=end
  ...
  def Vcard.decode_integer(v) # :nodoc:
  ...
  def Vcard.decode_date_list(v) # :nodoc:
    Vcard.decode_list(v) do |date|
      date.strip!
      if date.length > 0
        Vcard.decode_date(date)
      end
    end.compact
  end

  # Convert a RFC 2425 time-list into an array of times.
  def Vcard.decode_time_list(v) # :nodoc:
    Vcard.decode_list(v) do |time|
      time.strip!
      if time.length > 0
        Vcard.decode_time(time)
      end
    end.compact
  end

  # Convert a RFC 2425 date-time-list into an array of date-times.
  def Vcard.decode_date_time_list(v) # :nodoc:
    Vcard.decode_list(v) do |datetime|
      datetime.strip!
      if datetime.length > 0
        Vcard.decode_date_time(datetime)
      end
    end.compact
  end
  ...
  def Vcard.encode_text_list(v, sep = ",") #:nodoc:
    begin
      v.to_ary.map{ |t| Vcard.encode_text(t) }.join(sep)
    rescue
      Vcard.encode_text(v)
    end
  end
  ...
  def Vcard.encode_paramtext(value)
    case value
  ...
  def Vcard.encode_paramvalue(value)
    case value
    when %r{\A#{Bnf::SAFECHAR}*\z}
      value
  ...
  # line.
  def Vcard.decode(card) #:nodoc:
    content = Vcard.unfold(card).collect { |line| DirectoryInfo::Field.decode(line) }
  end
  ...
  # inner].
  def Vcard.outer_inner(fields) #:nodoc:
